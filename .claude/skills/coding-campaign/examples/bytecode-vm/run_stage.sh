#!/usr/bin/env bash
# run_stage.sh: one supervisor agent, one generation.
# Stage adapter over run_fleet.sh: stage prompt, compose_and_test.sh fitness over frozen
# substrate, then commission/score/rank.
# Invoker reviews results.tsv; may override champion.
set -u
