...
using TestAudioRing = ano::SpscRing<AnoAudioCommand>;
static_assert(offsetof(TestAudioRing, head) - offsetof(TestAudioRing, tail) >= ANO_CACHE_LINE,
...
static_assert(alignof(TestAudioRing) >= ANO_MIN_LINE,
               "audio SPSC ring must be cache-line aligned");
static_assert(alignof(ano::ByteSpscRing) >= ANO_MIN_LINE,
               "audio byte SPSC ring must be cache-line aligned");
