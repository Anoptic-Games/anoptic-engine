...
#undef LINT_CHECK
    ...
    //
    // Each config generates MATRIX_BARS + MATRIX_LOOKAHEAD bars but judges only
    // the first MATRIX_BARS: the extra contexts let an obligation planted in the
    // last judged bar (a cadential 6/4) discharge onto the V that follows it,
    // instead of being reported as unkept merely because the render stopped.
    // The event digests are identical with and without the lookahead — generation
    // is causal, so the extra bars cannot reach back into the window.
    //
    // All 120 lint clean, both stages, all seven families — TECH_SPEC 14.4's
    // "zero violations" contract, green against the CPython oracle. Getting here
    // took three fixes, each found BY this matrix: the D3 split-6/4 bar voiced
    // its own blocks without applying the C4 monophonic dyad rule (a real music
    // bug, fixed in both codebases); a truncated render reported obligations it
    // had no room to keep (the lookahead above); and the harmonic rules judged
    // modifier-displaced onsets at where the jitter left them rather than the
    // grid slot they were written on (fixed in both linters).
    {
...
// The judged window, and the extra bars generated purely so an obligation
// planted in bar 31 (a cadential 6/4) can discharge onto the V that follows it.
...
#define MATRIX_LOOKAHEAD 2
        ...
        for (uint32_t row = 0; row < MATRIX_N; ++row) {
            ...
            for (int bar = 0; bar < MATRIX_BARS + MATRIX_LOOKAHEAD; ++bar) {
                ...
                // past the horizon we keep the CONTEXT and drop the events: the
                // lookahead bars exist only to discharge obligations planted in
                // the last judged bar, and they must host nothing to judge
                if (bar < MATRIX_BARS) {
    ...
    // --- the long run: past every wall the state used to have ---
    // The phrase/bar caches were flat arrays indexed by ABSOLUTE index, capped
    // at 128 phrases / 64 scheduled segments / 1024 bars, with bounds guards.
    // Past those the guards did not crash — they silently stopped firing, so a
    // session longer than ~20 minutes quietly lost its form machinery while the
    // music played on. They are now direct-mapped rings tagged with the owning
    // index (music_form.h), so the index stays absolute (it spells the RNG
    // stream tag) while the storage wraps.
    //
    // Python's caches are dicts and have no such wall, which makes it the ground
    // truth for what the music should be out there. This walks 1200 bars — 155
    // scheduled phrases — under a repeating affect cycle that keeps the
    // dramaturg spending and the texture rotating, and compares every bar from
    // 1000 on: raw and post-chain digests, event counts, and the phrase position
    // (which is what a stale segment ring would corrupt first).
    {
...
// bars: 1200 (old ANO_MAX_BARS was 1024)
        ...
        // the walls are genuinely crossed: 1200 bars is 155 phrases, and the old
        // caps were 128 phrases / 64 segments / 1024 bars
