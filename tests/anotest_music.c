...
// Exact golden compare; a mismatch prints both bit patterns (platform FP forensics).
static bool feq(double a, double b)
{
    if (a == b)
        return true;
    fprintf(stderr, "feq: got %a (%.17g) want %a (%.17g)\n", a, a, b, b);
    return false;
}
...
int main(void)
{
    ...
    // --- interleaved mixed draws (rejection, two-word random, gauss cache) ---
    {
        AnoMusicRng m;
    ...
    // ======================================================================
    // Theory kernel vs prototype vectors
    // ======================================================================

    // --- functional walk: 24 scripted bars, fresh per-bar streams ---
    {
    ...
    // --- voicing chain: voicings + float costs ---
    {
    ...
    // --- IR tier: Meter vs musicgen/ir.py ---
    // bar_of: floordiv + banker's round; off-grid and 1-ULP-adjacent starts.
    {
    ...
    // Affect sweep (exact doubles); deadbands: pick_mode, gate_layers, pick_instruments.
    {
        static const struct {
            double v, e, ten;
        ...
        // Oracle emits patch names (not raw ids).
    ...
    // rough_cell tagged streams; short-circuit gates (merge/split/first-drop).
    {
    ...
    // --- motif tier vs gen/melody.py + gen/motif.py ---
    // Factories: rough_cell -> choice -> randint; realizers keep first max transposition.
    {
    ...
    // Empty clock div/mod; dramaturg walk idle/withhold/spend + ledger.
    {
    ...
    // Director: fresh/observed/aged, leniency extremes, forced request; imitation entry/retry/doubling.
    {
    ...
    // Scripted walks; events exact (start, dur hexfloat, pitch, vel, role, tie, degree).
    {
...
#undef MEV_OK
    ...
    // --- generators: arp + perc + pad + counter vs gen/*.py ---
    // Arp skip/A2/updown/6-8; perc groove/fills/compound; pad block/ties; counter over live lines.
    {
#define GEV_OK(ev, X) \
    (feq((ev).core.start, strtod((X).s, NULL)) && feq((ev).core.dur, strtod((X).d, NULL)) \
     && (ev).core.pitch == (uint8_t)(X).p && (ev).core.velocity == (uint8_t)(X).v \
...
#undef GEV_OK
    ...
    // Tie exemptions, echo mask, luftpause/pickup, agogic, compound swing; plain/perform chains.
    {
#define MOD_OK(ev, X) \
    (feq((ev).core.start, strtod((X).s, NULL)) && feq((ev).core.dur, strtod((X).d, NULL)) \
     && (ev).core.pitch == (uint8_t)(X).p && (ev).core.velocity == (uint8_t)(X).v \
...
#undef MOD_OK
    ...
    // Per-bar Blake2b-8 digests (raw + post-chain), counts, tempo, chord. W1 default; W2 everything-on + affect script.
    {
    ...
    // --- theory linter vs verify.py (section 8.4) ---
    // Same walks through linter as (rule, bar) multiset. W1 clean; W2 three real verdicts; L3 corrupted IR fires rules.
    {
...
#define L2_PRE_N 1
static const LintVio L2_POST[] = { { NULL, 0 } };
#define L2_POST_N 0
...
// (rule, bar) multiset vs golden, order-independent.
...
// Every rule group on one walk.
#define LINT_ALL(P)                                                                  \
    do {                                                                             \
        ano_lint_report_reset(&rep);                                                 \
        ano_lint(RAW, nraw, CTX, nctx, -1, mtr, ANO_LINT_PRE, NULL, &rep);               \
        LINT_CHECK(P##_PRE);                                                         \
        ano_lint_report_reset(&rep);                                                 \
        ano_lint(FIN, nfin, CTX, nctx, -1, mtr, ANO_LINT_POST, NULL, &rep);              \
        LINT_CHECK(P##_POST);                                                        \
        ano_lint_report_reset(&rep);                                                  \
        ano_lint_groove(RAW, nraw, CTX, PAR, nctx, -1, mtr, &rep);                        \
        LINT_CHECK(P##_GROOVE);                                                       \
        ano_lint_report_reset(&rep);                                                  \
        ano_lint_outer(RAW, nraw, CTX, nctx, -1, mtr, 0.5, &rep);                         \
        LINT_CHECK(P##_OUTER);                                                        \
        ano_lint_report_reset(&rep);                                                  \
        ano_lint_periods(RAW, nraw, CTX, nctx, -1, mtr, &rep);                            \
        LINT_CHECK(P##_PERIOD);                                                       \
        ano_lint_report_reset(&rep);                                                  \
        ano_lint_texture(RAW, nraw, CTX, PAR, nctx, -1, mtr, &rep);                       \
        LINT_CHECK(P##_TEX);                                                          \
        ano_lint_report_reset(&rep);                                                  \
        ano_lint_imitation(RAW, nraw, CTX, nctx, -1, leng.st.imitationTag,             \
                           leng.st.imitationCells, mtr, 0.9,                       \
                           &rep);                                                     \
        LINT_CHECK(P##_IMIT);                                                         \
    } while (0)
...
#define LINT_ACCUM()                                                            \
    do {                                                                        \
...

        {
            ...
            // Corrupted raw IR: rules must fire.
            ...
            ano_lint_report_reset(&rep);
            ano_lint(BAD, nraw, CTX, nctx, -1, mtr, ANO_LINT_PRE, NULL, &rep);
            LINT_CHECK(L3_PRE);

            // Rules W2 skips: obligations, counterpoint, groove, texture, cadence.
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                // Mapper moves shaping params every bar: groove contract off; pin fingerprint.
            ...
            ano_lint_report_reset(&rep);
            ano_lint(BAD, nraw, CTX4, nctx, -1, mtr, ANO_LINT_PRE, NULL, &rep);
            LINT_CHECK(L4_PRE);
            ano_lint_report_reset(&rep);
            ano_lint_groove(BAD, nraw, CTX4, PAR4, nctx, -1, mtr, &rep);
            LINT_CHECK(L4_GROOVE);
            ano_lint_report_reset(&rep);
            ano_lint_texture(BAD, nraw, CTX4, PAR4, nctx, -1, mtr, &rep);
            LINT_CHECK(L4_TEX);
            ano_lint_report_reset(&rep);
            ano_lint_imitation(BAD, nraw, CTX4, nctx, -1, leng.st.imitationTag,
                               leng.st.imitationCells, mtr, 0.9, &rep);
            LINT_CHECK(L4_IMIT);
        }
...
#undef LINT_CHECK
    ...
    // --- TECH_SPEC 14.4 acceptance matrix vs CPython ---
    // 12 seeds x dramaturg on/off x 5 RELEASING affects = 120 configs, 32 judged bars (+ lookahead for cadential 6/4 discharge).
    // Per config: raw/final digests + lint multiset, both stages. Zero violations.
    {
...
static const MatrixRow MATRIX[] = {
    ...
    { 2, 1, 1, 6172520351296540501ULL, 10958244341797607514ULL, 0, 16476032584258269876ULL },
    { 2, 1, 2, 253304391706424154ULL, 15464051639079502161ULL, 0, 16476032584258269876ULL },
    { 2, 1, 3, 16002106295852254565ULL, 3380513426159992159ULL, 0, 16476032584258269876ULL },
    { 2, 1, 4, 6454533556804121647ULL, 1405578048630449018ULL, 0, 16476032584258269876ULL },
    ...
    { 3, 0, 3, 14977019834560244270ULL, 2242229112813133380ULL, 0, 16476032584258269876ULL },
    ...
    { 3, 1, 1, 2199038387103972408ULL, 4465984907253750189ULL, 0, 16476032584258269876ULL },
    { 3, 1, 2, 7945449463137107889ULL, 16902543763847457245ULL, 0, 16476032584258269876ULL },
    { 3, 1, 3, 3187640105413153653ULL, 2888495627724353264ULL, 0, 16476032584258269876ULL },
    { 3, 1, 4, 12029861642220337377ULL, 5143014348923264685ULL, 0, 16476032584258269876ULL },
    ...
    { 4, 1, 3, 3820541168522608874ULL, 14336800813142289697ULL, 0, 16476032584258269876ULL },
    ...
    { 6, 0, 3, 10026346494410875931ULL, 9863008502277912481ULL, 0, 16476032584258269876ULL },
    ...
    { 6, 1, 1, 12362527691513888844ULL, 11511375774887132272ULL, 0, 16476032584258269876ULL },
    { 6, 1, 2, 5396539993592164951ULL, 15581498881440574020ULL, 0, 16476032584258269876ULL },
    { 6, 1, 3, 1083187010263572858ULL, 18126282493061215655ULL, 0, 16476032584258269876ULL },
    { 6, 1, 4, 2353162344165515646ULL, 15811519148870876775ULL, 0, 16476032584258269876ULL },
    ...
    { 7, 0, 4, 11596595932719381292ULL, 17444547997141269828ULL, 0, 16476032584258269876ULL },
    { 7, 1, 0, 8501354444765914205ULL, 9083797849648071926ULL, 0, 16476032584258269876ULL },
    { 7, 1, 1, 2688214233752382707ULL, 12865247069998181181ULL, 0, 16476032584258269876ULL },
    { 7, 1, 2, 17444894708462338510ULL, 11720422722546499562ULL, 0, 16476032584258269876ULL },
    { 7, 1, 3, 9727268129672980071ULL, 6571223963745676988ULL, 0, 16476032584258269876ULL },
    { 7, 1, 4, 10956741663290794351ULL, 1467583645499226289ULL, 0, 16476032584258269876ULL },
    ...
    { 8, 0, 3, 906507618502134603ULL, 1753209953642291121ULL, 0, 16476032584258269876ULL },
    ...
    { 8, 1, 1, 13929125655326294416ULL, 7953927387040721403ULL, 0, 16476032584258269876ULL },
    { 8, 1, 2, 16978931167202975415ULL, 1602280404117270845ULL, 0, 16476032584258269876ULL },
    { 8, 1, 3, 5781832814476013486ULL, 14099458877449157459ULL, 0, 16476032584258269876ULL },
    { 8, 1, 4, 8908661410992699863ULL, 10866892978233843898ULL, 0, 16476032584258269876ULL },
    ...
    { 9, 1, 1, 4120663711335273876ULL, 3712501771073718468ULL, 0, 16476032584258269876ULL },
    { 9, 1, 2, 2728340673427619298ULL, 14923052515774347854ULL, 0, 16476032584258269876ULL },
    { 9, 1, 3, 7587223925231502528ULL, 17898051241315301050ULL, 0, 16476032584258269876ULL },
    { 9, 1, 4, 5551417957544688575ULL, 14981079566062226995ULL, 0, 16476032584258269876ULL },
    ...
    { 10, 1, 1, 2685889471672743313ULL, 6377766088801949271ULL, 0, 16476032584258269876ULL },
    { 10, 1, 2, 10084810714469027213ULL, 15167261546476009918ULL, 0, 16476032584258269876ULL },
    { 10, 1, 3, 9974719669004831662ULL, 3120480357982589392ULL, 0, 16476032584258269876ULL },
    { 10, 1, 4, 3059356575032066665ULL, 2942382818033940503ULL, 0, 16476032584258269876ULL },
...
//   seed 10 dram 1 traj 4: [('cadential64', 31)]
        ...
        // (bar, valence, energy, tension); each point sets all three axes.
...
// Judged window + lookahead bars for bar-31 cadential 6/4 discharge.
#define MATRIX_BARS      32
#define MATRIX_LOOKAHEAD 2
        ...
        for (uint32_t row = 0; row < MATRIX_N; ++row) {
            ...
            for (int bar = 0; bar < MATRIX_BARS + MATRIX_LOOKAHEAD; ++bar) {
                for (int k = 0; k < 4; ++k) {
                ...
                // Past horizon: keep context, drop events (lookahead is discharge-only).
                if (bar < MATRIX_BARS) {
                    memcpy(MRAW + nr, mres.rawEvents,
                           mres.rawEventCount * sizeof MRAW[0]);
            ...
            ano_lint(MRAW, nr, MCTX, nc, MATRIX_BARS, mmtr, ANO_LINT_PRE, NULL, &mrep);
            ano_lint(MFIN, nf, MCTX, nc, MATRIX_BARS, mmtr, ANO_LINT_POST, NULL, &mrep);
            ano_lint_groove(MRAW, nr, MCTX, MPAR, nc, MATRIX_BARS, mmtr, &mrep);
            ano_lint_outer(MRAW, nr, MCTX, nc, MATRIX_BARS, mmtr, 0.5, &mrep);
            ano_lint_periods(MRAW, nr, MCTX, nc, MATRIX_BARS, mmtr, &mrep);
            ano_lint_texture(MRAW, nr, MCTX, MPAR, nc, MATRIX_BARS, mmtr, &mrep);
            ano_lint_imitation(MRAW, nr, MCTX, nc, MATRIX_BARS, meng.st.imitationTag,
                               meng.st.imitationCells, mmtr, 0.9, &mrep);

            // (rule, bar) multiset hashed as the oracle hashes it.
            typedef struct
            {
                const char *rule;
                int         bar;
    ...
    // --- long run past old flat-cache walls (rings in music_form.h) ---
    // 1200 bars / 155 phrases; compare bars 1000+: digests, counts, phrase position.
    {
...
static const LongRow LONG[] = {
    { 1000, 4, 22, 22, 4991964934079488028ULL, 2118847097695645109ULL },
    { 1001, 5, 27, 27, 12150706279091984051ULL, 11883921045277435801ULL },
    { 1002, 6, 28, 28, 4174242849859728753ULL, 11796282045416500123ULL },
    { 1003, 7, 22, 22, 2029053600954772106ULL, 17122890866109204255ULL },
    { 1004, 0, 23, 23, 11871097465736370155ULL, 5687972986964777594ULL },
    { 1005, 1, 23, 23, 10781479210613206065ULL, 16027311586540025413ULL },
    { 1006, 2, 14, 14, 548023312344925510ULL, 3496585206853530398ULL },
    { 1007, 3, 21, 21, 846715424914250337ULL, 8618477209169195271ULL },
    { 1008, 4, 22, 22, 490908474279784235ULL, 14393068046276904742ULL },
    { 1009, 5, 19, 19, 4975053892591182421ULL, 1086140043942023926ULL },
    { 1010, 6, 24, 24, 14037896935732447619ULL, 10577202859117138358ULL },
    { 1011, 7, 22, 22, 7392492313688845151ULL, 9275550034441976043ULL },
    { 1012, 8, 23, 23, 15058232544338543766ULL, 15910074713478770578ULL },
    { 1013, 9, 20, 20, 8476615094413275034ULL, 12007849589356460919ULL },
    { 1014, 0, 22, 22, 4876831171654810366ULL, 11676098432618828302ULL },
    { 1015, 1, 20, 20, 9691476955121831989ULL, 14431556356135951263ULL },
    { 1016, 2, 15, 15, 2451629938816108398ULL, 9221966850426676079ULL },
    { 1017, 3, 20, 20, 2349401509542280332ULL, 4833723576590867536ULL },
    { 1018, 4, 19, 19, 3403134120420194021ULL, 5705355533531696049ULL },
    { 1019, 5, 21, 21, 15406354873188236821ULL, 12305804118812278621ULL },
    { 1020, 6, 24, 24, 1713217606755952929ULL, 2205003271952172212ULL },
    { 1021, 7, 22, 22, 14053485877022264622ULL, 824223709956202036ULL },
    { 1022, 0, 20, 20, 956854482266018453ULL, 3269479694881892681ULL },
    { 1023, 1, 21, 21, 4630129947306229712ULL, 5572127522957447965ULL },
    { 1024, 2, 31, 31, 5673462123851170340ULL, 14661254124148393690ULL },
    { 1187, 4, 25, 25, 3765891720610635750ULL, 12023237032742450502ULL },
    { 1188, 5, 28, 28, 6791595398399195313ULL, 10698874567666136328ULL },
    { 1189, 6, 29, 29, 15655199942028120346ULL, 16386438560271589616ULL },
    { 1190, 7, 21, 21, 16357235042985864387ULL, 11950414793065595803ULL },
    { 1191, 0, 22, 22, 14525373019951986904ULL, 14887545485784971216ULL },
    { 1192, 1, 23, 23, 11956486962394134935ULL, 7561037215197343505ULL },
    { 1193, 2, 16, 16, 16141287889092992203ULL, 2318991166630488920ULL },
    { 1194, 3, 21, 21, 11472846144979268111ULL, 5440338207667529957ULL },
    { 1195, 4, 21, 21, 18243422193440660800ULL, 2107662214434542160ULL },
    { 1196, 5, 22, 22, 370528751934482366ULL, 3439706450846974742ULL },
    { 1197, 6, 25, 25, 16464632472761749059ULL, 9731698718070903310ULL },
    { 1198, 7, 22, 22, 17657063346615350903ULL, 7864701577219795302ULL },
    { 1199, 0, 17, 17, 11982502428018405716ULL, 5658456695887099173ULL },
};
#define LONG_N    200
#define LONG_BARS 1200
#define LONG_FROM 1000
// phrases scheduled: 155 (old ANO_MAX_SEGMENTS was 64, ANO_MAX_PHRASES 128)
// bars: 1200 (old ANO_MAX_BARS was 1024)

        static const double LCYCLE[6][3] = {
            { 0.30, 0.50, 0.45 }, { -0.60, 0.35, 0.80 }, { 0.70, 0.85, 0.30 },
            { 0.10, 0.65, 0.90 }, { -0.20, 0.25, 0.15 }, { 0.50, 0.75, 0.60 },
        };
        static AnoMusicEngine leng2;
        static AnoBarResult   lres2;
        ...
        cfg.form.bassInversions = cfg.form.split64 = true;
        cfg.texture.doubling = cfg.texture.animate = cfg.texture.imitation = true;
        cfg.texture.rotate = cfg.texture.counter = true;
        cfg.ties.anacrusis = cfg.ties.suspension = cfg.ties.syncopation = true;
        cfg.clock.codetta = cfg.clock.extension = cfg.clock.elision = true;
        cfg.melody.planApex = cfg.melody.counterpoint = true;
        ano_engine_init(&leng2, 42, &cfg);

        uint32_t row = 0;
        for (int b = 0; b < LONG_BARS; ++b) {
            if (b % 32 == 0) {
                const double *a = LCYCLE[(b / 32) % 6];
                ano_engine_set_affect(&leng2, a[0], a[1], a[2], false);
            }
            ano_engine_advance_bar(&leng2, &lres2);
            if (b < LONG_FROM)
                continue;
            const LongRow *g = &LONG[row++];
            uint64_t rd = ano_events_digest(lres2.rawEvents, lres2.rawEventCount);
            uint64_t fd = ano_events_digest(lres2.events, lres2.eventCount);
            bool ok = lres2.bar == g->bar && lres2.context.phrasePos == g->phrasePos
                      && (int)lres2.rawEventCount == g->rawc
                      && (int)lres2.eventCount == g->finc && rd == g->rawd
                      && fd == g->find;
            if (!ok) {
                printf("  long bar %d: pos %d/%d raw %u/%d fin %u/%d digests %d/%d\n",
                       g->bar, lres2.context.phrasePos, g->phrasePos,
                       lres2.rawEventCount, g->rawc, lres2.eventCount, g->finc,
                       rd == g->rawd, fd == g->find);
                CHECK(ok, "long-run bar");
                break; // one report is enough; the rest would cascade
            }
        }
        CHECK(row == LONG_N, "long-run coverage");
        // Walls crossed: 1200 bars / 155 phrases (old caps 128/64/1024).
