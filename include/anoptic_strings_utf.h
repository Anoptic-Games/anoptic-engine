...
// UTF-16 only at OS boundaries. Malformed bytes -> U+FFFD, advance one. Total, no error paths.
// Simple 1:1 case mapping. Positions are byte offsets.
// Collation/case/class cover: Latin European, Greek, Cyrillic, Runic, kana, Han (code point), punct.
// Everything else falls back to code point order.
...
// Decode ending just before *i, move *i to its start. *i == 0 -> U+FFFD, stays.
...
// General category Nd: ASCII 0-9 and fullwidth digits.
...
// General category P*. Symbols (S*) are not punctuation.
...
// Script primary order: punct < digits < Latin < Greek < Cyrillic < Runic < kana, then unlisted by code point.
int anostr_collate(anostr_t a, anostr_t b);

// First four nonzero primary weights of s, big-endian u64. Unequal keys agree with collate's sign.
...
// Sort values in collation order, in place. Stable.
...
// Sort interned symbols by collation order, in place. Stable. Mutates the per-symbol key cache.
...
// Byte index of first base-letter match of needle at/after `from`. Empty needle -> min(from, len).
