...
Sibling of tools/perf/bench_fps_win64.py. The platform-agnostic methodology (engine log contract, foreground/DPI/warmup rules, reading the numbers) lives in tools/perf/README.md.

Linux-specific:
  - window discovery by PID via xdotool search --pid (GLFW sets _NET_WM_PID), name fallback
  - forced + verified foreground via xdotool windowactivate --sync + getactivewindow
  - borderless exact-size render surface via _MOTIF_WM_HINTS strip + xdotool windowsize --sync
  - 'M' menu toggle synthesized with XTEST (xdotool key)
...
X11 hands out device pixels: window sizes here are physical. The swap= VRAM check in tools/perf/README.md stays the resolution authority.

Tools (bring them in with NIX, never apt): xdotool (required), wmctrl + xprop (optional). e.g.:
  nix shell nixpkgs#xdotool nixpkgs#wmctrl nixpkgs#xorg.xprop nixpkgs#xorg.xrandr
Wayland: drives X11/Xwayland clients only. Run the engine as an X11/Xwayland client (DISPLAY + X server).
...
CHURN_MS = 33.0  # target resize cadence floor
...
def _find_window(pid):
    """Window id for pid: _NET_WM_PID first, GLFW title 'Vulkan' cross-checked by pid as fallback. Decimal ids throughout."""
...
def _strip_decorations(wid):
    # Motif hint flags=MWM_HINTS_DECORATIONS(2), decorations=0 -> borderless.
...
def _toggle_menu(wid):
    # Focus, then XTEST 'm' to the focused window.
...
def main():
    ...
    env = dict(os.environ)
    # Hide WAYLAND_DISPLAY from the child: engine comes up as an Xwayland client (--env can restore it).
