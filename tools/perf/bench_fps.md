...
The engine emits three lines to `anoptic.log` (written next to the executable), all release-visible, defined in `src/vulkan_backend/frame/profiling.c`. Wall-clock timing is tallied in a single `anoperf_accumulator_t` — one `ano_timestamp_us` read per presented frame in `ano_frame_mark()` (inlined in `frame.h`), the dt against the previous frame's stamp stored into a fixed window, no extra clock read — and every **ANO_PERF_WINDOW_FRAMES** = 128 presented frames the accumulator flushes: sorted once, logged as a `[frame]` + `[frametime]` pair, reset. Nothing is ever logged per frame; 128 = 2^7 and spans ~2 s of frames at a 60 fps target, ~1 s at 120.
