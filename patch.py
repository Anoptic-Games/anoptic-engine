...
with open("src/render/gltf/ano_GltfParser.c", "r") as f:
    content = f.read()

# Remove createCombinedVertexBuffer and createIndexBufferForMesh
import re

content = re.sub(r'bool createCombinedVertexBuffer.*?return true;\n}', '', content, flags=re.DOTALL)
content = re.sub(r'bool createIndexBufferForMesh.*?return true;\n}', '', content, flags=re.DOTALL)

# Replace processGltfMeshes
new_process = """
...
content = re.sub(r'void processGltfMeshes.*?\n} ', new_process, content, flags=re.DOTALL)

with open("src/render/gltf/ano_GltfParser.c", "w") as f:
    f.write(content)
