...
=======
// FROZEN SEAM (blueprint 2.2). Placement: WHERE a byte lives. The model is DATA -- a
// res_model literal in resources_models.c -- and res_place_plan() is the only routing
// code in the module. No plan_hook, no `if (model == ...)`, ever.
//
// The placement vocabulary (role/operation/destination/provenance) lives here because a
// route plane is indexed by it. Consumers never see a res_site: they build a res_place_plan
// (resources_internal.h) and call res_owned_alloc().
>>>>>>> block-b1-base
...
// The routing input. Defined in resources_internal.h; only pointers cross this header.
typedef struct res_place_plan res_place_plan;

// ---------------------------------------------------------------------------------------------
// The placement vocabulary. WHO asks (role), WHY (operation), WHAT SHAPE the byte wants
// (destination), WHERE IT CAME FROM (provenance). Each _COUNT bounds a route-plane axis.
>>>>>>> block-b1-base
...
=======
    RES_DEST_CHUNK,             // fixed RMOS_CHUNK_MAX streaming chunk (D13: contested)
>>>>>>> block-b1-base
...
=======
    RES_PROVENANCE_PACK,        // came out of a pack: validate() is MANDATORY on adopt
    RES_PROVENANCE_COUNT,
} res_provenance;
...
typedef struct res_arena_spec {
    uint8_t       backing;        // res_backing; RES_BACK_NONE -> plan() REFUSES, loudly
    uint8_t       confers;        // RES_SITE_* this arena grants
    uint32_t      min_block, max_block;             // multipool (geometric)
    const size_t *classes; size_t class_count;      // multipool (explicit histogram; D20)
>>>>>>> block-b1-base
...
=======
// The model roster, resolved by name (ANO_RES_PLACEMENT). NULL when the name is unknown.
const res_model *res_model_by_name(const char *name);
const res_model *res_model_default(void);
...
// PURE ROUTE: reads the plan, allocates nothing, mutates nothing, returns the site.
// Blueprint 2.2 names this res_place_plan(). In C that identifier is already the plan TYPE
// and the two share one namespace, so the VERB is res_place_route(). The type keeps the
// frozen name, because every consumer writes it.
>>>>>>> block-b1-base
