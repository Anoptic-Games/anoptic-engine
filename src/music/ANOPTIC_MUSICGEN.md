...
arrows never reverse. `anoptic_audio.h` contains no music type even though it carries
`ACMD_MUSIC_*` commands: those are scalars.
...
byte-identical events. Three things depend on it:
...
- A seek can be "rebuild the engine off-thread and adopt it": a rebuild lands on
...
Every draw is a fresh stream, tagged. A draw is seeded from `(seed, tag, bar)`
(`"42:mod:melody:17"`), not pulled from one long sequence. Parity is not "did we draw the
same number of times overall"; it reduces to *condition gating* and *state-mutation order*.
Get the branch right and the number is right.
...
music. Only the storage wraps. A slot carries the index that owns it, and a read that
...
chord, cadence, motif landing. They arrive when the bar **sounds**, not when it was
...
The linter is a tripwire. Digests prove *the same thing happened*, not that it was any good.
It found three real bugs the digests could not.
...
- Modifier-displaced onsets. A strum or a humanize can nudge a note *backwards* across a
  ...
  jitter left it. Exact: no modifier moves a note by half a grid step.
