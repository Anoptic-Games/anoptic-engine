...
// mutex. The caller drains it to the sink via ano_log_flush() on its own schedule, so the logger
// owns no thread. FATAL and _now write straight through. Stop all producers before ano_log_cleanup.
...
{
    size_t need = len + 1;
    bool full = g_bufLen + need > ANO_LOG_BUF_CAP;
    if (full) {
        if (g_fullPolicy == ANO_LOG_DROP_NEWEST) {
            g_dropped++;
            return -1;
        }
    ...
    g_buf[g_bufLen + len] = '\n';
    g_bufLen += need;
    if (full && g_fullPolicy == ANO_LOG_FULL_IMMEDIATE) {
        flush_locked();
...
int ano_log_init(void)
{
    ...
    g_fullPolicy = ANO_LOG_FULL_IMMEDIATE;
