...
// UI per-tile prim lists: CPU-coarse stage (ui-render.md §3.7). Dense 8px tile grid.
// Each prim scatters into overlapping tiles (counting sort). Solid bit = full coverage.
// Pure, any thread.

// PERF TODO (ui-render.md §3.7.3-4, deferred):
//  - opaque-truncation: opaque solid OVER resets its tile list to just it
//  - sparse dispatch: non-empty tiles + compact active-tile list
//  - GPU binning: move scatter to compute
...
// Opaque RRECT fill fully covering tile [tx0,ty0)-(tx1,ty1)? Exact coverage-1 core test.
...
// In: scene, grid origin + tile counts, caller buffers.
// Out: offsets (prefix-summed) + prim-index entry stream (painter order, solid bit per tile).
// cursor = tilesX*tilesY scratch. *ok false if a cap is too small.
                           ...
                           uint32_t *cursor, bool *ok)
{
    ...
    *ok = true;
    ...
    // Pass 1: count each prim into offsets[tile+1].
    ...
    // Prefix sum: offsets[t] = tile t start, offsets[nTiles] = total.
    for (uint32_t t = 0; t < nTiles; t++)
    ...
    // Pass 2: scatter prim indices (ascending), tag solid coverage.
    for (uint32_t t = 0; t < nTiles; t++)
        cursor[t] = offsets[t];
    for (uint32_t i = 0; i < s->primCount; i++)
    {
