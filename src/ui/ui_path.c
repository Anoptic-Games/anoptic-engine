...
// UI path baker: contours (lines + quads) -> directed monotone quads in the sweeper stream.
// Grammar matches text bake: p0 (p1 p2)+ per contour, ANO_UI_CURVE_SENTINEL between.
// Points are packed binary16 in prim-LOCAL space (origin = bbox center).

// Fill: nonzero-winding, winding-independent. Opposite-wound inners punch holes.
...
// Center-local point -> stream word (binary16 x low, y high).
static uint32_t pack_pt(double x, double y)
{
    return (uint32_t)ano_half_pack((float)x) | ((uint32_t)ano_half_pack((float)y) << 16);
}

// In: count packed words (in == out legal), scale s > 0. Sentinels (+inf) survive.
...
// Emit monotone pieces as (p1 p2) pairs into curves[*w]. False on overflow. Bumps *segs.
                          ...
                          uint32_t flags)
{
    ...
    // Pass A: quads in builder space, contour boundaries, bbox. Lines -> straight quads.
    ...
    for (uint32_t i = 0; i < segCount; i++)
    {
        ...
        // bbox over endpoints and controls
    ...
    // Shift to prim-local (bbox center), accumulate endpoint shoelace.
    ...
    // Reverse if shoelace > 0: curve_area fills when outer has non-positive shoelace (y-down).
    ...
    for (uint32_t c = 0; c < cn; c++)
    {
        ...
        // Contour start (p0), then each quad's monotone pieces.
        if (w + 1 > cap)
            return ANO_UI_REF_NONE;
        if (!reverse)
        {
