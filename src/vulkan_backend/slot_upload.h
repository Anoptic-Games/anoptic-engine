...
// SlotUpload: ×1 DEVICE_LOCAL per-slot GPU buffer fed by a per-frame host-visible delta staging
// ring, plus the slot-indexed entity-buffer growth path. Render-thread only. See slot_upload.c.
...
// device DEVICE_LOCAL buffer + N host-visible staging buffers + region arrays; false on failure.
...
// Record frame f's staged deltas as a copy into the device buffer (called from the record path).
void slot_upload_flush(VkCommandBuffer cmd, SlotUpload* b, uint32_t f);
// Grow every slot-indexed GPU buffer (and the slot ceiling) to hold >= required slots; false on OOM.
