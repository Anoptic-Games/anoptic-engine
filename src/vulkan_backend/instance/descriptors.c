...
// Bind froxel buffers: global 10/11 (fragment) + light-cull 0-4 (in UBO/runtime/lights, out count/index). Init-only.
...
// (Re)bind per-mip Hi-Z build sets after resize. 0=pyramid sample, 1=mip storage, 2=depth reduce source. Live hizMipCount only.
void updateHiZDescriptorSets(VulkanContext* ctx, RendererState* state)
{
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
        for (uint32_t v = 0; v < ANO_VIEW_COUNT; v++)
        {
            ...
            for (uint32_t m = 0; m < vr->hizMipCount; m++)
            {
                ...
                // Binding 2 reduce source: depthMaxResolve -> resolve view, else MSAA depth (sampler2DMS). Layout SHADER_READ.
    ...
    // Cull binding 11: cull samples pyramid built lag submits earlier (1 in-frame, 2 async). SHADER_READ. Re-run on resize.
    for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
    {
