...
// Clustered-forward froxel grid. ANO_CLUSTER_MAX_LIGHTS caps lights per froxel.
...
// UI overlay lane table capacities (per-frame uiFrameBuffer regions).
...
// Default per-view cull threshold (px). 0 disables.
#define ANO_CULL_PIXEL_THRESHOLD_DEFAULT  1.5f

// Default per-view LOD threshold (px). 0 disables.
#define ANO_LOD_PIXEL_THRESHOLD_DEFAULT   128.0f

// Default shadow LOD offset vs view 0.
#define ANO_SHADOW_LOD_BIAS_DEFAULT       0

// Dynamic shadow: one cull frustum per map. Moment maps (RGBA16), CDF lower bound.
...
// Runtime shadow headroom above static rig: single (dir/spot), point (6 faces).
...
// Layered Power CDF: CDF_LAYERS depth bands. Keep in sync with shadow_cdf.glsl.
...
// Data shapes only (not allocated/bound/drawn yet).
...
// Per-instance bone matrices for visible skinned set; GPU-regenerated each frame.
...
// Per-view state within a frame-in-flight. View 0 = main camera.
typedef struct ViewResources
{
    ...
    // Hi-Z views: all-mip sample, per-mip storage, per-mip sets.
    VkImage             hizImage;
    ...
    // Per-view sets: global, lightcull, tonemap.
...
typedef struct PerFrameResources
{
    ...
    // Picking readback (view 0). Host-visible|coherent, persistently mapped.
    ...
    // View-independent sets: cull/update/scatter.
    ...
    // Text overlay image + host-visible textFrameBuffer.
...
typedef struct RendererState
{
    ...
    // Overlay scale: logical -> framebuffer px (0 until window = treat as 1). Main-thread write.
    float                   uiScale;
    ...
    // Per-view extent: view 0 = swapchain; aux = inset.
    ...
    // Tonemap pass (HDR->swapchain). Standalone, not a PipelineType.
    ...
    // Text overlay. Gate textOverlay: off on ANO_FORCE_NO_TEXT or bake init failure.
    bool                    textOverlay;
    ...
    // Pending OSD text canonical; textVersion bumps on change.
    ...
    // Logic-submitted text blocks; each entry adopts the allocation.
    ...
    // UI overlay lane. Gate uiOverlay rides textOverlay; off on ANO_FORCE_NO_UI / init fail.
    bool                    uiOverlay;
    ...
    // Hi-Z pyramid build. Pipeline PIPELINE_COMPUTE_HIZ; hizSetLayout shared per-mip.
    ...
    // Dynamic shadow pass (moment maps). Reuses cull-compacted draw lists.
    ...
    // Transient shadow occluder depth: one slice per frustum.
    VkImage                 shadowDepthImage;
    GpuAllocation           shadowDepthAlloc;
    VkImageView             shadowDepthSliceView[ANO_SHADOW_FRUSTUM_COUNT];

    // CDF shadow atlas + blur temp: one instance across FIF. RGBA16, ANO_SHADOW_ATLAS_LAYERS (2 sublayers/frustum).
    ...
    // Dirty-frustum cache. shadowCacheMode: 0=normal, 1=always dirty, 2=freeze.
    ...
    // Swept-bound motion exposure: CPU mirrors of base pose + mesh index. Arrays [slotMotionCap].
    ...
    // Content-dirty re-render budget/frame. Matrix-dirty exempt. 0 = unlimited.
    ...
    // Shadow config SlotUploads + shadowCfgMirror (render-thread CPU copy for gating).
    ...
    // Static rig frustum allocator: fills [0, shadowFrustumNext).
    uint32_t                shadowFrustumNext;
    uint32_t                shadowTypeUsed[3];

    // Runtime frustum free-lists above static rig (single dir/spot; point = 6-block bases).
    ...
    // ECS <-> render bridge. Render owns slot authority via render_slots.
    ...
    // Per-view cull threshold (px). 0 disables. -> CullUBO.viewCullParams[v][1] squared.
    float                   cullPixelThreshold[ANO_VIEW_COUNT];
    // Per-view LOD threshold (px). 0 disables. -> CullUBO.viewCullParams[v][2].
    float                   lodPixelThreshold[ANO_VIEW_COUNT];
    // Global LOD bias (+ coarser). -> CullUBO.viewCullParams[v][3].
    int32_t                 lodBias;
    // Shadow LOD offset vs view 0 (+ coarser). -> CullUBO.shadowLodBias.
    int32_t                 shadowLodBias;
    // Hi-Z: viewProjHist -> CullUBO.prevViewProj; hizEnable per-view (0=off).
    mat4                    viewProjHist[MAX_FRAMES_IN_FLIGHT][ANO_VIEW_COUNT];
    uint32_t                hizEnable[ANO_VIEW_COUNT];

    // Async Hi-Z: compute pyramid, cull lag-2. gfxTimeline/hizTimeline 1-based. Gate: asyncHiz.
    bool                    asyncHiz;
    ...
    // Async light-cull on compute during shadow. preludeTimeline/lcTimeline order split. Gate: asyncLc.
    bool                    asyncLc;
    VkSemaphore             preludeTimeline;
    VkSemaphore             lcTimeline;

    // Task meshlet cull (flat.task). Set at init: meshShader && taskShader && !ANO_FORCE_NO_TASK.
    bool                    taskCull;
