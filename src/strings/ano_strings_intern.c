...
// The runtime interning table: one canonical copy per distinct string, dense u32 symbols.
// Open addressing (power-of-two capacity, linear probing) over cached 64-bit hashes; the
// canonical values and their hashes live in dense per-symbol arrays, so growth rehashes
// nothing and never touches the string bytes. Everything -- the table struct, the slot and
// symbol arrays, the canonical bytes -- allocates from the table's heap and dies with it;
// there is deliberately no destroy function (region granularity, like the whole module).
//
// Threading: single mutator, same rule as the mi_heap underneath. Readers (find/sym_str/
// count) are safe alongside each other but need external ordering against the mutator.
...
// The symbol holding (hash, s), or ANOSTR_SYM_NONE at the first empty slot.
...
// Double the slot table and reinsert from the cached hashes -- no string bytes touched.
...
static int grow_arrays(anostr_intern_t *t)
{
    ...
    if (hashes == NULL)
        return -1;
    t->hashes = hashes;    // committed independently: a failed strs growth still leaves
    anostr_t *strs = mi_heap_realloc(t->heap, t->strs, (size_t)newCap * sizeof *strs);
    if (strs == NULL)
        return -1;         // arrCap unchanged, so the larger hashes block is just slack
    t->strs = strs;
...
anostr_sym anostr_intern(anostr_intern_t *t, anostr_t s)
{
    ...
    // Insert path. Grow first so failure leaves the table exactly as it was.
    ...
    if (canonical.len != s.len)         // keep's only failure mode: a failed long copy
        return ANOSTR_SYM_NONE;
...
anostr_t anostr_dedupe(anostr_intern_t *t, anostr_t s)
{
    ...
    if (sym == ANOSTR_SYM_NONE)
        return s;   // table unavailable: the caller's value is still perfectly usable
    return t->strs[sym];
}
