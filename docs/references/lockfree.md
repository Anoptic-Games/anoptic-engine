...
Companion to `docs/logger.md` §2 (the per-thread-lanes vs shared-ring decision) and `docs/notes.md` §2 / Step 5.
...
3. Morrison & Afek, "Fast Concurrent Queues for x86 Processors," PPoPP 2013. The FAA pivot — LCRQ/CRQ. Read for: FAA to pick a cell + an almost-always-uncontended per-cell CAS, rings linked by M&S at the rare boundary. This is the paper that answers your "I had no idea about FAA." PDF: https://www.cs.tau.ac.il/~mad/publications/ppopp2013-x86queues.pdf
