...
The book spends ~9 pages on SMR: once a node is unlinked, when is it safe to free, given other threads may still hold pointers into it? The techniques, in escalating sophistication:
...
SMR applies when nodes are dynamically allocated and freed. An array-based structure that reuses slots in place does not.
...
- Enqueue is a two-step update: CAS the last node's `next` to the new node (the linearization point), then swing `tail` (cleanup; any thread can help). Dequeue linearizes at the CAS that moves `head` (p. 148).
- Consistent snapshot: enqueue re-reads `tail` after reading `tail.next` and relies on the counted-pointer increment to prove the two reads were simultaneously valid. Same "check, then let the CAS validate" shape the logger's multi-slot claim uses (p. 149).
...
A nonblocking operation must be total (defined on any state), so a dequeue on empty returns ⊥. Often a thread needs to wait for a precondition: an empty queue to fill, or a full bounded queue to drain (the book lists "a full bounded container" explicitly, p. 179). The naive answer, spin `repeat v := remove() until v ≠ ⊥`, wastes cycles and, worse, lets the scheduler pick who gets the next datum (p. 179).
...
- The preemption tax (p. 78 table; §7.5.2): every fair/queued lock rates poorly on preemption tolerance. A descheduled lock-holder, or a descheduled thread in the middle of the wait queue, stalls its successors. Inherent to lock-based mutual exclusion under preemption.
...
Lock-free queue literature moved from CAS to FAA as its core primitive. Post-2013 queues (LCRQ, SCQ, wCQ) scale where Michael & Scott (1996) does not. `docs/notes.md` already reaches for it ("reserve with `fetch_add` on the tail") without naming the lineage. This is that lineage, the hardware underneath, and the map onto Anoptic's logger, event bus, and cache-line-stripe.
...
A bare FAA is already wait-free. A queue whose common path is a single FAA is wait-free on that path for free.

---

### 3. The contention truth: FAA avoids *wasted* traffic

FAA does not sidestep cache-coherency cost. The counter's cache line still has one owner at a time under MESI; it ping-pongs between cores, one acquisition per `fetch_add`. A single hot FAA counter is still a serialization point and a scalability bottleneck.

Precise statement: FAA does *useful work on every line acquisition* (owner completes its increment and leaves with a ticket); a CAS loop *wastes line acquisitions* (acquire, read, compute, try write, find stale, fail, re-acquire). FAA converts unavoidable serialization into linear productive serialization; CAS turns it into superlinear waste.

Engine cache-line-stripe (`notes.md` §2): one FAA = one unavoidable line bounce, so make each bounce carry more payload. Claim a 64-byte stripe with one FAA, fill with N plain stores, publish with one release. Per-item coherency cost falls to ~1/N. Same move as a sharded/striped counter (`LongAdder`-style): when one FAA line is too hot, split into per-core lines and sum on read.
...
Heuristic: if the hot operation can be phrased as "atomically take a unique number," it is an FAA; if it must be phrased as "atomically replace A with B only if it is still A," it is a CAS. Modern queue designs (§5) reframe the queue so the hot path is the first kind and only the rare slow path uses the second.

---

### 5. The queue lineage

Each step's one-line contribution. Through-line: push CAS off the common path; make the common path an FAA index into an array.
...
Suggested order: dependency order. Each builds on the last; read M&S first so the FAA pivot in LCRQ lands, then SCQ/wCQ as its refinements.
...
- choll/xtr: the closest single prior art to `docs/logger.md`: delegate all formatting and I/O to one background thread, minimize call-site work, deferred-format over a bounded ring. It diverges exactly where we did: per-sink SPSC lanes (the Quill side of the §2 decision). Rests on two C++ tricks that do not port to C: a per-call-site function pointer the compiler generates from the argument types (so a no-arg log is "one pointer to the ring"), and fmtlib. Its one borrowable idea is the double-mapped "magic" ring buffer (`mirrored_memory_mapping`: map the buffer twice back-to-back so a wrapping variable-length record is contiguous, deleting spill/reassembly); SPSC-coupled, so revisit it alongside the lanes question for the ECS/event streams. ~2 ns no-arg, C++20 + fmtlib, Linux/x86-leaning (TSC, optional io_uring). https://github.com/choll/xtr
