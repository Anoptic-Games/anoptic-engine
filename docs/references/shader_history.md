...
> Scope note: "Apple Silicon usable" = native Metal capability.
> Engine talks Vulkan: practical gate is the translation layer
> (MoltenVK today, KosmicKrisp later). See [Notes for the Anoptic Engine](#notes-for-the-anoptic-engine).
...
### ② Mesh graphics pipeline: modern geometry front-end
...
- Tess Control + Tess Eval are all-or-nothing: both or neither.
- Classic and Mesh front-ends are strictly mutex. A pipeline is either
  {Vertex, Tess, Geometry} or {Task, Mesh}, never both. (Lighting-branch crux.)
- Fragment is shared: after the rasterizer regardless of front-end.
- Compute is its own pipeline; never a stage beside graphics stages.
...
1. "Apple Silicon usable" = native Metal. Engine talks Vulkan: real gate is the translation layer. Metal has mesh shaders since Ventura (2022); MoltenVK does not expose `VK_EXT_mesh_shader`. Mesh-shader renderer cannot run on macOS Sonoma despite capable hardware. KosmicKrisp / macOS 26 dependency.
2. Geometry shaders are a dead end on Apple: no Metal stage, no MoltenVK, no KosmicKrisp. Any `geometryShader` device-feature requirement is cruft and hostile to macOS if nothing uses the stage.
