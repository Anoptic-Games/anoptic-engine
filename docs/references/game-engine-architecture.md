...
- ✅ The job-system design (thread pool, `uintptr_t` param, counter-based join, spin-locks) is exactly the C-friendly, no-mutex approach Anoptic wants for Step 8.
...
Medium priority, for the Step 7 renderer rewrite. We use Vulkan directly and **deliberately skip PBR**, so the relevant material is the pipeline architecture and the **high-entity-count** techniques.
