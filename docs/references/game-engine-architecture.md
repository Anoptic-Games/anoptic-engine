...
- ✅ Start-up/shut-down: in **C this is free** — there are no constructors to misorder. Our modules already use explicit `ano_*_init()`/`ano_*_cleanup()` (e.g. [anoptic_logging.h](../../include/anoptic_logging.h)). Keep `main()` (the main loop) calling them in explicit dependency order; this is Gregory's recommended pattern with the C++ caveat deleted.
