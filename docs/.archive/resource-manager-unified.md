...
// Whether any mount currently contains the logical path. Advisory only.
// Metadata caches lie (9P/SMB). Prefer ano_res_load + NULL blob.
bool ano_res_exists(const char *logical);

// -- The read path --------------------------------------------------------------------------

// A loaded resource as a value. data == NULL (and size == 0) means the load failed.
// Bytes live in the caller heap and die with it. No free function
// (anostr_intern_t lifetime discipline).
typedef struct {
    void  *data;
    size_t size;
} ano_res_blob;

// Load a whole resource through the mount table.
// ANO_CACHE_LINE-aligned (SPIR-V uint32_t pCode + GPU-staging grain), one 0x00 guard past end.
// size = bytes read to EOF, never stat(). No failure path is UB.
...
// NEVER retried on the same handle (fsyncgate); caller's buffer is source of truth.
// Call again to retry.
...
// via the full protocol to a brand-new filename "saves/<slot>.<seq>.anosave".
// A name that never existed cannot have a stale 9P/SMB cache entry. Verified via a fresh
// read handle before older generations are pruned (keep ANO_RES_SAVE_KEEP). Same-slot
