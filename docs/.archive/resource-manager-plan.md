...
**House premise:** the hard part is already in-tree. The logger *is* an async system: lock-free MPSC ring in, owned consumer draining behind the callers, 22–48 ns enqueue, TSan-clean, fuzz-oracled. The render bridge ships SPSC ownership transfer. The async tier below is a port of shipped machinery, not new design.
...
hardcoded path it kills. **Tests** = the battery.
...
- **Lands:** `include/anoptic_resourcemg.h`; `src/resourcemg/` (platform-free core +
  ...
  (exe + `resources/`).
- **Deletes:** nothing yet. Pure capability.
...
- **Lands:** the logger's variable-length MPSC ring moves to `anoptic_collections.h`
  as a generic ring (the logger re-consumes it. One ring, two users). On it: request
...
- **Tests:** range correctness vs whole-file oracle (random offset/length fuzz); pool
  ...
  raw drive bandwidth** (prove or revert codec work).
...
- **Tests:** bake determinism (same input → byte-identical pack); TOC bit-flip refuses
  ...
  model renders identically to the cgltf path. FontTools-oracle culture applied to
...
- **Bar to even start:** `anotest_resbench` shows rung 0 leaving the drive idle while
  ...
  io_uring/IOCP remain a recorded rung below this one.
