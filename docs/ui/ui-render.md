...
How the general UI rendering layer (panels, buttons, menus, layering, images, shadows/glow) fits this renderer and 2024-2026 GPU vector graphics.

Short version: the text lane should become the UI lane. Scanline Sweeper coverage (`textcoverage.glsl`) is already a general fill rasterizer over monotone quadratic outlines; `curve_area`/`window_sum` are not glyph-specific; holes and annular borders fall out of the signed-area sum; the per-instance clamp survives self-overlap.

Missing: runtime-submitted geometry, typed-primitive layer, closed-form shadows/glow, paints (gradients/textures), clip/z model, scaling past single-bbox dispatch.

Present: async overlay lane, composite insertion, id-keyed block bridge, bindless textures, input back-channel.

Field survey (§2): production UI converged on "typed analytic primitives + painter's order + CPU-side layout"; Vello 2025 pivoted to CPU-coarse / GPU-fine, the split this renderer already has.
...
Six schools, each scored against this renderer. Sources in §9.
...
Production answer for editor-grade UI. GPUI renders exactly five primitive classes:
...
exactly right; steal both. Implement them inside the compute lane we already have rather
...
and complexity far past what a UI workload needs. Do not build a general scene
...
Linebender's own pivot (classic Vello memory unpredictability + downlevel-GPU
...
beta as of 2026 Q1. This is the strongest external validation of the recommended
...
per pixel; fine for boundary tiles, ruinous for interior pixels, so §3.7's interior
classification is required if this path is taken. Default: closed-form rrect; outline
lowering as a quality escape hatch.

Two shapes sharing an exact edge produce
...
Closed-form shadow math (GPUI ships this; CC0):
...
- Scope limit: axis-aligned rrect-family only. Shadows of rotated
...
What the UI lane inherits without modification (anchors current):
...
3. GPU plumbing, visually inert: buffers, descriptor writes, pipeline, empty dispatch wired
   ...
   cross-session ms vary with desktop load per the usual A/B-within-session rule). Live
...
6. Gradients + clip table + dither + UI_PATH (runtime monotone-quad bake through the text
   ...
   The self-test scene gained a gradient strip + a play-triangle path (both
...
7. Per-tile lists + interior/opaque classification at compose, indirect dispatch, glyph
   ...
   tiles, and the §3.7.4 GPU binning pass.)
...
Engine-internal: docs/text/font-render.md (design doc for the lane being generalized),
