...
Resolution check: swap VRAM tracks pixel count linearly, ~170-188 MiB per megapixel across the sweep. The 4K row's swap is 3.9x the 1080p row against a 4.0x pixel ratio; every row rendered at its true labeled resolution. Swap, not the window label, is the authority. Swap column byte-identical to the 2026-07-11 runs (unchanged renderer).

## vs 2026-07-11-crashreport.md (3e6259e): the flagged regression does not reproduce

Same machine, same branch, three commits apart (3e6259e -> 4abd042; engine-relevant delta is c9e99f2 Perfbench refactor; f9f22fa docs, 4abd042 test-only). The 2026-07-11 run read ~20-30% below the feature-profiling baseline and flagged it for investigation before merge. Today's numbers recover fully and land on top of that baseline (2026-07-11-bench2.md, feature-profiling 781158e):
...
Every row matches or edges past feature-profiling; GPU-pass at or below profiling-era cost; frametime consistency improved (4K 1% low 216.3 -> 562.7 fps, 0.1% low 196.4 -> 550.4). Pre-merge concern in 2026-07-11-crashreport.md closed: 4abd042 shows no regression vs profiling merge-base. Cause of the 07-11 deficit (Perfbench refactor vs machine state) not separable. Conditions differ (bench2 15 s/point, 07-11 crashreport 45 s/point, this run 30 s default); rows line up by render extent.
