...
## Race cluster 1: seqlock value copy at `src/audio/audio_bridge.h:151` and `:162`
...
Seqlock even/odd protocol detects torn copies and retries, but value bytes use plain `uint8_t` loads/stores concurrently. C11: concurrent non-atomic access is a data race (UB); TSan flags every overlap. Highest-frequency report; alone fails `anoptic_audio`, `audiotone`, and `audioscene`.

Deliberate verbatim twin of `ano_seqpub_store`/`ano_seqpub_load` in `src/render_bridge/render_bridge.h:113-134` (private headers do not cross modules, so the code was copied). Twin has the identical formal race; `anotest_render_bridge` passes TSan only because its schedule never overlaps store and load on the same lane. Any fix must land in both twins.

## Race cluster 2: cross-thread `AnoSynth` reset at `src/synth/ano_synth.c`
...
No protecting protocol: `transport_start` memsets the voice pool and zeroes both queue index pairs while the mixer may be mid-drain. On ARM the mixer can observe `cmdHead` from the new epoch with `cmdTail` from the old; `tail < head` gate and `% QUEUE` arithmetic index garbage. Real corruption path; matches scene suites dying on macOS without TSan.

Ownership contract (`synth_internal.h:7`: logic thread while idle, generator thread once started) is violated by the engine itself: `src/engine/main.c` calls `ano_synth_transport_start(g_synth, ...)` on the logic thread after `ano_audio_init` has already started the mixer thread with the generator callbacks registered. Same shape the failing tests exercise.

Two adjacent latent races in the same cluster, not yet TSan-flagged only because the runs die first: producer-side overflow policy writes the consumer's index. `evtTail` advanced by producer on overflow (:537-538) while consumer drains it (:771); likewise `cmdTail` (:853-854 vs :1202). Two writers to one plain index breaks SPSC discipline in steady state, independent of transport resets.
...
- `anoseqpub` would reintroduce cluster 1 verbatim. Publish: odd-marker, release fence, `memcpy(p->value, v, stride)`, even release. Read: acquire, `memcpy(out, p->value, stride)`, acquire fence, recheck. Same plain-byte concurrent copy, same C11 data race, same TSan signature at `audio_bridge.h:151`. Branch never noticed: `anotest_collections` stresses only the rings across threads; `test_seqpub_unit` publishes and reads on one thread; tests workflow has zero runs on that branch. Word-lane fix applied here is exactly what `anoseqpub` needs before it can host either bridge (value plane is parent-allocated at init, so switching to `_Atomic uint64_t` words is contained; stride needs rounding to 8 or a `%8` precondition).
- The rings remediate nothing; nothing was broken there. Bridge ring payload memcpys are ordinary SPSC slot handoffs: ownership transfers through the acquire/release cursors, so plain copies are already defined behavior. `anoring_spsc` is field-for-field the same design as `AnoAudioRing`/`AnoSpscRing`, down to the `(tail - head) > mask` full test. Migration value is deduplication only: three private ring copies exist today (`audio_bridge.h`, `render_bridge.h`, `log_ring.h`) and the promotion both bridges promise would delete two.
- Cluster 2 is untouched by any primitive in the module: ownership-protocol violation (logic thread resetting mixer-owned state), not a missing interconnect. `anoseqpub` could carry the start request as a latest-wins `{epoch, worldFrame}` mailbox, but it is strictly heavier than the two-atomic handshake now in place (heap plane + version word + retry loop versus one added `_Atomic`), and the exactly-once epoch consumption on the mixer side is needed either way. Synth's internal evt/cmd queues need no lock-free structure: both ends run on the mixer thread; `anoring_*` would add atomic traffic to a single-threaded path.
...
## Cause 1: macOS headless: `-ffp-contract=off` was a silent CMake no-op; arm64 fused fmadd broke CPython bit-parity

16 failures are contraction: `a*b + c` fuses to `fmadd`. Failing maps (`ano_map_density`/`velocity`/`articulation`/`reverb_send`) are mul+add+clamp only; `ano_map_filter_cutoff` with `pow(2, x)` passed.

Music declared `-ffp-contract=off` ("bit-parity with CPython float op order", src/music/CMakeLists.txt). `set_source_files_properties` without `TARGET_DIRECTORY` only applies to targets defined in the same directory; `anoptic_core` is root-defined, so the property was ignored. x86-64 baseline has no FMA; arm64 baseline `fmadd` made macOS the first platform where contraction fired. `src/log/CMakeLists.txt:17` uses `TARGET_DIRECTORY anoptic_core` correctly; music's omission was the bug.

`ano_music_gauss` runs Box-Muller through libm `log`/`cos`/`sin`; Apple arm64 libm vs glibc ULP agreement unresolved until a CI run with the contract fix. `feq` now prints `%a` bit patterns on mismatch. Corrects "Why each CI job fails": cluster 2 never manifested on macOS; scene suites passed there all along.

## Cause 2: ubuntu tests-tsan: starvation on 2-core runners, not races

TSan costs 5-15x; GitHub runners have 2 cores; ctest runs suites in parallel against fixed per-test timeouts. `musicdrive` (26s healthy) blows its timeout; timing-sensitive suites (`anoptic_time`, heartbeat waits in `musicscene`) miss deadlines. Races this file documents are gone: no TSan report survives even under deliberate 2-core stress.
