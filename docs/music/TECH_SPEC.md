...
Technical spec for porting the musicgen prototype into the Anoptic engine: data structures, APIs, scheduling, state, determinism, audio-library requirements, and conformance. THEORY_SPEC.md owns the musical rules; when this doc says *what a field means musically*, THEORY_SPEC wins. The prototype landed on a finished IR and rule set (REFINEMENT_PLAN A1–D3 complete, PLANS.md M27); everything marked **contract** here was settled by building and listening.
...
The prototype (`musicgen/`, Python ≥ 3.12) has exactly one hard dependency: `mido`, confined to MIDI file I/O. **Everything above MIDI output (IR, theory kernel, generators, conductor, linter) is dependency-free**; the generation core ports to C directly. The synthesis stack (signalflow) is not ported; it was the shopping list for the engine's in-house C audio library (§12). SYNTHESIS.md records what it actually needed.
...
- **The grid**: `GRID = 0.25` beats (a 16th). Every pre-modifier event's start
  ...
  float-exact in binary; float time does not drift.
...
MIDI is the output, not the working representation; it cannot carry "this E5 is a passing tone over V7." The IR can. All inspection and linting operate on it. Schema is a candidate engine music-event format; annotations support engine-side debugging.
...
sidecar out of shipping builds while keeping it in dev builds. "why did it play that?" (§13) is a requirement.
...
Phrase-scoped RNG keys (§8.2) keep those decisions stable across the skew.
...
`bar += 1`. The generator emission order exists for same-bar data
...
output discarded. Generation is microseconds per bar; this is the intended
...
`signature`. Phrase-scoped keys make groove pinning, apex planning,
...
thread the sole graph owner with a block-boundary command queue: the
...
after the port; the rule set and its verification are locked above.
