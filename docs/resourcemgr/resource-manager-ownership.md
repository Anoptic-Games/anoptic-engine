...
Anoptic is C and uses arena allocators. Resource ownership follows the arena that owns the data; it is not modeled with C++ lifetime semantics.
...
Resource handles travel through a lock-free Vyukov queue. Its hot cursors are cache-padded like the logger's head and tail. The queue transports handles between subsystem owners; it does not make generic RAM the permanent owner of the resource.
