...
`resource-manager-SoA.md`; this document is the keepers only, ascending, each step
with its test battery and merge bar.
**Premise:** the hard part is already in-tree. The logger *is* an async system:
...
hardcoded path it kills (proof the old path is gone). **Tests** = the battery.
...
- **Lands:** `include/anoptic_resourcemg.h`; `src/resourcemg/` (platform-free core +
  ...
  site), `ano_res_resolve`/`_subpath`/`_exists` (prefer `ano_res_load`; these are for
  parsers that open files themselves, documented as such),
...
- **Tests:** new `anotest_resourcemg` (labels `unit;mem`): logical-path validation is
  total (fuzz bad paths: backslashes, `..`, leading `/`, overlong; all refuse, none
...
- **Bar:** CWD is irrelevant to the engine; nothing but the resource manager and the
  logger's append stream opens a file by path (grep can enforce it).
...
- **Tests:** frame round-trip and corruption battery in `anotest_resourcemg` (truncate,
  ...
  save_load never returns garbage (the fault-injection FS smoke test); config round-trip +
...
- **Lands:** the logger's variable-length MPSC ring moves to `anoptic_collections.h`
  as the generic ring (the logger re-consumes it; one ring, two users). On it: request
  ring (payload `{rid or path, offset, length, band, ticket}`; **ranges in the format
  from day one**), one IO thread (the drainer's park/wake and shutdown discipline
  verbatim), blocking `pread` + `posix_fadvise`, per-consumer SPSC completion rings
  drained by a per-frame pump. Tickets are `{u32 idx, u32 gen}`, false-on-full (render
  bridge convention). **Two bands: NOW and LATER**: a blocking wait on a LATER ticket
  boosts it; four-band scheduling and byte-budget metering stay rejected until a
  starvation bench exists. Missing file completes FAILED through the ticket, identical
  to sync.
...
- **Tests:** range correctness vs whole-file oracle (random offset/length fuzz); pool
  ...
  raw drive bandwidth** (compression as bandwidth amplification, proven or
...
- **Bar to even start:** `anotest_resbench` shows rung 0 leaving the drive idle while
  ...
  io_uring/IOCP remain a recorded rung below this one, expected unused at
...
- `kill -9` at every protocol step: every user file old-complete or new-complete; a
  torn newest save degrades one generation: quiet for the player, loud in the log.
