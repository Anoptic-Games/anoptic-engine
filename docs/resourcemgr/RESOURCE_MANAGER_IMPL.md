...
`docs/resourcemanager-comprehensive.md` supersedes the completion claims and forward contracts in `docs/resourcemanager-real.md`; the older document remains owner-directive provenance and historical planning. This journal remains a chronological record, so the earlier text and numbers stay intact while this entry corrects their current interpretation.
...
Phase F landed reusable SPSC/MPSC/SPMC/MPMC rings, a unique-number dispenser, byte publication, bridge policies, and waiter tests. It did not land a resource owner, request grammar, generational ticket lifecycle, public async API, completion system, or end-to-end resource transport. The current SPMC pre-claim stall window, `anoseqpub` ordinary-byte publication, shared-producer reserve-to-publish gap, and mutex/condition-variable waiter remain unsuitable for the final resource path until the comprehensive Stage C proofs and replacements land.
