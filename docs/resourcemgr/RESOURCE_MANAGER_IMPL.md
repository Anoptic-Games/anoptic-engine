...
Working journal for the resource-manager implementation. The current specification is `docs/resourcemanager-comprehensive.md`; this file records chronology and does not weaken that plan.

Earlier entries preserve what landed, what was measured, and what was believed at the time, including claims corrected by later entries. Phases A–F remain historical records rather than current completion certificates.
...
# Reconciliation — the comprehensive plan becomes authoritative (2026-07-13)

`docs/resourcemanager-comprehensive.md` supersedes the completion claims and forward contracts in `docs/resourcemgr/resourcemanager-real.md`; the older document remains owner-directive provenance and historical planning. This journal remains a chronological record, so the earlier text and numbers stay intact while this entry corrects their current interpretation.

Phases A–C landed real memory-pool, namespace, handle, registry, durable-write, save, graphics-ingest, font, shader, and renderer-integration machinery. Those capabilities are useful scaffolding, not the complete owner model, allocator hierarchy, resource-domain set, streaming/pack path, or ticket transport required by the comprehensive plan.

Phase D compared two exact prototype placements: the global-pool scaffold and the scoped-pool scaffold. The published measurements remain evidence for those implementations, but they were not the complete five-model contest: B, C, and D did not compete, the scoped path lacked the full C×D hierarchy, registry metadata and large payloads remained outside it, and production opened no scoped lifetime domains. Ordinary engine loads therefore remain in immortal group 0. The old closing-state table is an at-the-time verdict against the superseded plan, not current completion status; no allocator model has shipped or won the complete contest.

Phase F landed reusable SPSC/MPSC/SPMC/MPMC rings, a unique-number dispenser, byte publication, bridge policies, and waiter tests. It did not land a resource owner, request grammar, generational ticket lifecycle, public async API, completion system, or end-to-end resource transport. Stage C of this single task replaces the current SPMC pre-claim stall window, `anoseqpub` ordinary-byte publication, shared-producer reserve-to-publish gap, and mutex/condition-variable waiter with the proved final resource path.

The registry mutex and global save mutex are current scaffold facts, not permanent public contracts. The public resource API remains subject to the comprehensive interface review; the former “V1 additions only” rule does not prevent removal or renaming of prototype paths. Saves preserve every generation unless the user explicitly deletes one, while the newest-64 normal-candidate and eight-temp-candidate load windows remain a correctness gap because enough invalid recent files can hide an older valid generation.

Live scaffold terminology now maps as follows: historical `ANO_RES_MODEL=A`, `RES_MODEL_A`, and `*_modelA` mean `ANO_RES_PLACEMENT=global`, `RES_PLACEMENT_GLOBAL_POOL`, and `*_global_pool`; historical default/`ANO_RES_MODEL=E` and `RES_MODEL_E` mean `ANO_RES_PLACEMENT=scoped`, `RES_PLACEMENT_SCOPED_POOL`, and `*_scoped_pool`. These names describe placement scaffolding only and carry no Model A or Model E claim.

---

# Checkpoint — milestone landings, the M16 descope, and the first adversarial review round (2026-07-14)

This entry records the state committed at this checkpoint. The tree carries M0–M5 complete, M14's core (pack runtime, ranged reads, codec hardening, deterministic builder, anopak CLI), and M11's framing half, all gated on native Windows Debug (`build.bat 5`, 40/40) with the placement matrix verified under `ANO_RES_PLACEMENT=scoped-pool` (default), `global-pool`, both short aliases, and the loud bogus-name refusal (§7.1: a bench cannot lie about what it measured).

M2 landed the extension registry: kinds are fourccs interned to dense ids sorted at freeze, so ids are a pure function of the tag set (D17), and `res_tag_from_path` walks the extensions' `classify()` — the last suffix switch in the core is gone. M3 landed the five-axis telemetry: 128-byte cells, line-0 hot path, cell 0 as the counted overflow bucket that voids a run's numbers (D5). M5 landed the live placement seam: `res_place_route` as pure router, the two scaffold models (`scoped-pool`, `global-pool`) as `res_model` literals, and the M5 bug-preservation law honored — TRANSFER sites still carry a NULL allocator (today's >1 MiB placement bug), the multipool path still under-delivers alignment past 4096 with the site reporting the truth (D15), and no site is winkable; all three die at M6, on purpose, so the scaffold is behavior-identical to what it replaced. M11's framing half landed `res_plane_layout` / `res_block_seal` / `res_block_open` with a 1073-line hostile battery: an independent reference validator, boundary truncations, structural mutations under repatched hashes, and a 12,000-iteration deterministic fuzzer. Provenance: roughly ninety percent of this code was salvaged from the campaign worktrees (the wt2 CORE diff, the BLOCK fleet champion and battery) and reconciled by hand with the landed M4; the ported routing oracle caught a real defect in the salvage before it landed (`res_ext_register` had lost the intra-list duplicate-tag check), which is the fitness rigs paying rent.

**The M16 descope.** By owner decision of 2026-07-14, the audio and script resource domains leave the Phase A bar. This reverses owner scope decision 1 recorded in `phasea/CHECKPOINT.md` §1 ("Audio and script get real resource domains with headless-but-genuine consumers"). The reasons, stated so the next reader does not relitigate them: the owner maintains ano, an external embedded-language project with its own ruled bootstrap (Steel, then a derived embedded C implementation), and the blueprint's 16-opcode fuel-capped stack VM is a hostile-bytecode socket-proof, not a language — the owner has ruled that the `'SBCD'` socket may eventually be filled from ano rather than by the placeholder VM, and audio rides the same descope. The frozen headers (`anoptic_audio.h`, `anoptic_res_audio.h`, `anoptic_script.h`, `anoptic_res_script.h`, freeze item 12) and the honest PENDING test placeholders stay in-tree untouched. The staged fleet material (canonical WAV decoder and mixer champions with their oracles; compiler and VM champions with their reference evaluator) remains in the campaign worktrees, inventoried in the battlelog. Consequences owned: W10 levels loses its gate on W8+W9, and the Phase B role-split workload loses the audio/script consumer personalities until re-scoped. M11's other half — the graphics scene version 2 with the load-time cross-reference validation pass, the highest-severity item in the ledger — remains OPEN; no draft of it ever existed anywhere.

**Adversarial review round one.** Four independent GPT reviews (gpt-5.6-sol, xhigh effort, one per landed module: CORE, BLOCK, STRIPE, STREAM) returned 34 findings. Eighteen were fixed in this checkpoint. Block framing: the gate and the sealer now refuse a base below the header's own alignment (a typed view of a skewed base is UB; absolute plane alignment stays the allocation's contract, base-relative grain stays the gate's). Pack runtime: entry validation refuses wrap-prone `raw_size`/`data_off` arithmetic before any offset math trusts it; chunk records must be in-order and non-overlapping within a read window; every read re-checks the mounted TOC generation (8 bytes per operation) so a pack rebuilt underfoot is refused rather than served under the old table; `rmos_size_hint` no longer vetoes a readable pack (it is a hint by contract — file truth is proven by reads); and the builder excludes its own output from the walk, so building into the source tree stays byte-deterministic. Registry: the reader-epoch protocol got a seq_cst publish-then-revalidate in `ano_res_read_begin` paired with seq_cst owner-side scans, closing a store-buffering window in which a payload could be reclaimed under a live read scope; `alive` became atomic; shutdown re-runs its idempotent retirement scan on every attempt (an OOM-interrupted first pass no longer strands surviving rows on retry); `ano_res_domain_retire` accepts a RETIRING domain so a failed retire is retryable; `ano_res_release` clears dependency storage like retirement does; `ano_res_stats_delta` subtracts through the object representation instead of UB pointer arithmetic; and row generations track a cross-init high-water mark so a stale `{rid, slot, gen}` handle can never validate against a reborn registry. Telemetry refuses out-of-field-width axes into the counted overflow bucket instead of masking them onto someone else's cell. Memory pools: fixed pools floor their internal alignment at `alignof(void *)` (free-list links live in the block), the counting parent acquires at its own header alignment and zeroes the ledger at birth, oversize frees uncharge the recorded truth rather than the caller's claim and count `parent_releases` only when a release path exists, and `requested_bytes` resyncs to zero at quiescence.

Ten findings were review-confirmations of wounds this journal and the blueprint already own, now with independent evidence: the M8 cache-hit alias, the M9 release-accounting family (zero-copy release leaves placement and telemetry gauges charged; `res_registry_external_allocation` charges and never reverses), the M14-integration stubs (`res_candidates_ex` pass 2, `ano_res_get_range`, pack-blind `ano_res_slurp`, the sink entry points), and domain teardown ignoring unpublished owned blocks plus the pack TOC surviving shutdown (M6–M9 domain surgery). Six were recorded as new wounds with owners: reader registration racing shutdown's lane destruction and the public stats/telemetry snapshot thread contract (M18 sweep — Stage A is owner-thread synchronous by construction, the API does not yet say so enforceably); the shutdown-retry-versus-unregister documentation mismatch (M18); the multipool explicit-class alignment wording for non-power-of-two classes at or above the cache line, where the header's promise and the validated behavior disagree (resolution owed at M6/M18 before any histogram-tuned class list ships); the counting-parent context with no destroy path over the default parent (an API extension, blocked by the freeze); and the epoch/counter wraps at 2^64, which are recorded as unreachable rather than defended. One finding was rejected with reasoning: zero-count planes occupying a positive span violate no admission rule of the frozen gate — canonical zero-span placement is `res_plane_layout`'s output contract, not an open() precondition, and the slack is hash-covered and extent-bounded like any other.

Verification cells for this checkpoint: Windows native Debug `build.bat 5` green (40 enabled tests), placement matrix as above, resbake/resplace/respack/mempools re-gated after every review fix. WSL ASan/TSan and the 9P floor were last run against the pre-checkpoint tree and are due a re-run; macOS remains UNRUN, as always, and is never claimed.
