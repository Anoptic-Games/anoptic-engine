...
- **`include/anoptic_<module_name>.h` — the contract.** Public surface. Constants, type aliases, and function signatures. This is all a caller ever sees. Functions surfaced by `anoptic_<module_name>.h` should always begin with `ano_` and are always lowercase.
- **`src/<module_name>/` — the secret.** The implementation details. It can contain private
...
`<module_name>_<platform>.c`, eg `timers_linux.c`, `timers_win64.c`, are platform-specific and can be dynamically selected by the CMakeLists.txt.
