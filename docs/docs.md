...
- **`include/anoptic_<module_name>.h`** Public contract. Constants, type aliases, function signatures. Callers see only this. Functions begin with `ano_`, always lowercase.
- **`src/<module_name>/`** Implementation. Private headers (`<module_name>_<platform>.h`) stay inside the module. Raw `pthread_*`/Win32 prototypes, internal structs, helpers never leak into `include/`.
...
❌ **Not anoptic** Implementation bleeding into the public header

✅ **Anoptic** Header signature stays `ano_xxxxx()`
