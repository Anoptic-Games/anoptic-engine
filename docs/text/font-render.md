...
(`docs/references/scanline_sweep.md`). Prerequisites already present:
a dedicated async compute queue with four timeline lanes, a lag-0 async pass
precedent (light-cull) with the sync shape text needs, cross-queue CONCURRENT
...
Work: one new async lane plus one new core module. No frame-graph surgery.
Friction points (§6) have known mitigations; two came from auditing the font file.
...
- All curve data is static: cubic→quadratic (not needed for TrueType), split until x- and
  ...
  never invalidated by scale, perspective, or orientation. Same buffers serve screen-space UI
  today and world-space decal/flat-geometry text later.
...
- One documented failure mode: overlapping same-winding contours break the area sum
  (coverage denominator exceeds 1). Live in our demo charset (§6.1).
...
(ONE, ONE_MINUS_SRC_ALPHA). Text composites over everything including PiP views,
post-tonemap in LDR.
...
- `anostr_t` (Step 4) shipped; `docs/TODO.md:28` gates the UTF meaning layer on
...
than render-side: shaping is what the logic world calls when text blocks arrive over the
bridge. FreeType allocations route through a module-owned mi_heap via custom `FT_Memory`
hooks (FreeType supports this natively).
...
Game code shapes into instance arrays wherever it runs; the caller-buffer signature writes
straight into a mapped per-frame buffer with zero allocation. penOut chains runs (color
changes mid-line) bitwise-exactly.
...
New async lane (mirrors light-cull, the lag-0 precedent; not Hi-Z, which is lag-2:
...
- Overlay image ×MAX_FRAMES_IN_FLIGHT (per frame, not per view; it overlays the final
  ...
  in `recreateSwapChain` (`instanceInit.c:1340`). ~40 MiB at 2560×1368 (§6.6).
...
overlay covers a few percent of the screen on a 3090; expect low tens of µs. Runs on the
compute queue concurrent with the shadow/geometry region (~2 ms of graphics work); awaited
only at the composite fragment stage. Same-frame data, fully hidden latency, same shape as
light-cull.
...
5. Multi-glyph pixel overlap forbids the naive per-glyph dispatch. Adjacent glyphs' bboxes
   ...
   required for correctness, not an optimization: each pixel is owned by exactly one thread,
   which iterates all covering instances and blends in registers.
...
inside the render thread: the stats live there, zero bridge traffic for the PoC. Demo
exercises shaping, baking, raster, async sync, and composite blend with content that changes
at a realistic cadence (numbers tick every 2 s) while the instance buffer re-uploads per
frame, matching the eventual UI workload shape. (120 frames is a frame count, not wall time:
a fraction of a second at demo framerates; make it a knob if the flicker rate annoys.)
Toggles: `ANO_TEXT_OVERLAY` (demo on/off), `ANO_FORCE_NO_ASYNC_TEXT` (fallback lane A/B).
Note the async lanes (hiz/lc/text) sit outside the graphics timestamp chain; text pass timing
uses the established freeze/A-B methodology rather than a new `ANO_TS_*` slot for now.
...
ACCEPTED: the block was packed and enqueued and ownership crossed into the bridge. A count above `ANO_RENDER_TEXT_MAX` is clamped and still ACCEPTS; a count-0 set tail-forwards to clear and returns that clear's answer.
...
## 9. Future paths (out of PoC scope)
...
8. Demo: profile-line mirror at print cadence, record before/after frame numbers here.
   ...
   noise; the raster itself runs on the async lane (step 7). The PoC goal (profile lines
   on-screen through the Scanline Sweeper) is delivered.)
