...
Both endpoints answer `AnoRenderSubmitResult`, not a bool, because a producer's correct reaction differs per outcome: ACCEPTED means the block was packed and enqueued and ownership crossed into the bridge (a count above `ANO_RENDER_TEXT_MAX` is clamped and still ACCEPTS, and a count-0 set tail-forwards to clear and returns that clear's answer); BACKPRESSURE means the command ring was full, the packed block was released, nothing was enqueued and retrying next tick is the policy; OOM means the block itself could not be allocated, which is never spelled as backpressure because retrying it at tick rate hammers an allocator that just refused; INVALID means count > 0 with a NULL instances pointer, is deterministic, and must retire the block rather than be retried. `ano_render_text_clear` allocates nothing and validates nothing, so its domain is ACCEPTED or BACKPRESSURE alone 〜 that is what lets a caller's exhaustive switch treat the other two as unreachable. Callers inspect `result.code` only; `if (result)` inverts success because ACCEPTED is 0, and switching without a `default` makes a fifth code a compile-time diagnostic at every policy site.
