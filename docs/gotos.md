...
`texture.c:450` 〜 `createTextureImage` (the docs/BUGS.md entry is anchored at `texture.c:486`, the first post-acquisition arm), and its twin `texture.c:381` 〜 `createTextureImageFromPixels`. Must WAIT for the resource-manager custody decision. These are two of the three anchors of the open custody chain (`texture.c:426`, `components.c:72`, `ano_GltfParser.c:277`), which six independent fix rounds each stopped in front of. Nobody converts them ahead of that ruling. Recorded here so the shape is not mistaken for an unblocked candidate: seven arms after the decode, discharging `texture.pixels`, `stagingBuffer`+`stagingAlloc`, `*textureImage`+`*textureImageAlloc`, `*textureImageView`. Note the hazard that makes it more than bookkeeping 〜 the staging buffer conditionally escapes through `*outStagingBuffer` at `:526`, which runs above the last arm, so a naive label double-frees. Whatever the custody ruling says, that line decides the label's discharge set.
...
Two defects surfaced by this sweep are not goto matters and belong in docs/BUGS.md rather than here, noted only so they are not lost: `pickPhysicalDevice` (`device.c:345`) has one failure arm that frees `devices` and strands `ctx->availableDevices` with every per-slot allocation from `:440`; and `createTextureImage`'s arms from `:504` down silently break the total-the-face contract its own first two arms honor, returning false with a live `VkImage` in the caller's out-param.
