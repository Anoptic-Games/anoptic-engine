...
Status: this is the sole current completion specification for the Anoptic resource manager. `docs/resourcemgr/resourcemanager-real.md` preserves the owner's directives and the superseded plan, `docs/resourcemgr/RESOURCE_MANAGER_IMPL.md` preserves implementation chronology rather than specification authority, and the remaining documents under `docs/resourcemgr/` contain historical research and planning.
...
The owner's directives in `docs/resourcemgr/resourcemanager-real.md` define the goal. `docs/resourcemgr/resource-manager-SoA.md`, `docs/resourcemgr/resource-manager-plan.md`, `docs/resourcemgr/resource-manager-unified.md`, `docs/references/lockfree.md`, and `docs/text/logger.md` provide research and historical design context. `docs/resourcemgr/RESOURCE_MANAGER_IMPL.md` is an implementation journal, not authority to weaken the goal. Where an older plan describes a caller-owned librarian, keeps three save generations, retains a registry mutex forever, or calls a reduced hierarchy complete, this plan supersedes that outcome.
...
- Stages A, B, and C are dependency order, not release boundaries or permission to stop. This effort continues through the functional baseline, all five complete allocator contestants, the selected production hierarchy, the total lock-free handle/ticket transport, migration of every production consumer, deletion of every superseded path, and the final platform/runtime evidence before it is called complete. No required item in this plan is postponed to a later project, follow-up, or unspecified future pass.
...
| Hierarchy scaffold | Commit `6455651` removed the false Model A/E executable labels. The current global-pool and scoped-pool paths remain one global multipool plus optional scoped multipools and a monotonic glTF scratch arena; registry metadata remains outside, bulk data uses direct blocks, role-specific pools/stripes do not exist, and production opens no scopes. | Preserve the truthful scaffold names until Stage B implements full Model A, B, C, D, and E. Full Model E is the complete C×D hybrid with real lifetime domains, role split, backing heaps, consumers, and zero-copy transfer classes. |
...
| Other resource domains | No complete world/level, config/keybinding, script, audio-resource, or save-migration consumer exists. The demo models remain hardcoded string literals in renderer startup. | Land real data-driven level/world, config/keybinding, script, audio, and save-migration resource domains with production consumers, quarantine/migration where applicable, and resource-declared dependencies/prefetch. Eliminate hardcoded demo asset ownership from renderer code. |
...
The hierarchy is nevertheless incomplete. Current production therefore runs the global-pool/group-0 scaffold: its multipool is immortal, production creates no lifetime groups, group pools have default-heap parents instead of owning heaps, rows/names/hash tables bypass the hierarchy, large resources bypass it, decoded images bypass it, fixed pools and stripes have no resource client, and pooled destructive release copies. Stage A preserves the real primitives while completing the neutral ownership grammar required to build every full contestant.

### A.4 Bring the system into line

- Maintain the terminology established by commit `6455651`: live code, tests, logs, and environment variables use global-pool/scoped-pool scaffold names, while superseded Model A/E claims remain only as explicitly annotated history.
...
- Remove every alternate asset IO and parsing path outside resource extensions, including legacy path-based font loading; tools and importers also enter through the resource namespace and extension grammar.
...
Implement the two promised bands, NOW and LATER, without head-of-line ambiguity. A blocked wait on LATER may publish a boost through the control path. Run the starvation, queue-depth, and backend evidence during this delivery; implement more bands, byte budgets, work stealing, parallel IO owners, io_uring, or IOCP wherever those results establish a requirement, otherwise preserve the raw rejection evidence. No required escalation is left as follow-up work.
