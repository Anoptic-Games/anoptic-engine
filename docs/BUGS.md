...
- pending-design-decision 〜 the defect is understood and its root cause is already tagged, but the repair is blocked on a contract choice with more than one defensible answer, so writing the fix would settle that question silently. Never the only tag on an entry. Write-ups live in docs/BUG-HUNT.md. Status tags are excluded from every root-cause tag count. The five 2026-07-25 carriers are settled under docs/BUGS_DONE.md, "Settled open decisions". The three 2026-07-26 product decisions (mixed colour/data textures, GPU-arena failure semantics, submission result API) are settled under docs/BUG-HUNT.md, "Settled decisions (2026-07-26)"; the tag currently marks nothing.
...
- amended (2026-07-26) 〜 arena reclaim and ownership transfers settled in docs/BUG-HUNT.md, "Settled decisions (2026-07-26)" §2; entry stays open until the custody state machine lands.
...
- silent-drop
- amended (2026-07-26) 〜 representation settled in docs/BUG-HUNT.md, "Settled decisions (2026-07-26)" §1: mutable-format image, separate SRGB/UNORM views, separate bindless indices; entry stays open until that lands.
...
- amended (2026-07-26) 〜 product decision settled in docs/BUG-HUNT.md, "Settled decisions (2026-07-26)" §3: `AnoRenderSubmitResult` on owned-payload text/UI/bulk endpoints, no bool wrapper; the audit expands the repair beyond the four HUD spins to `ano_render_ui_*`, `ano_render_submit_bulk_*`, and shutdown-aware `submit_blocking`. Entry stays open until that implementation lands.
