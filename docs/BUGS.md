...
- pipeline.c:54 〜 loadFile's ftell has no failure guard; a -1 becomes 0xFFFFFFFF and degrades into the allocation or short-read arm, both total since the 2026-07-25 loadFile fix, so the consequence is a clean refusal 〜 an explicit check would be hygiene, not a repair.
