...
Four further requirements come from the `notes.md` audit: batched output (one syscall per drain pass), a monotonic timestamp per record, a lossless full-ring path (write-through, §10), and a working output file.
...
// then: one ano_fs_write of the whole batch.
```
...
`ano_log_immediate` (FATAL, `_now`) writes on the calling thread instead of through the flusher. The full-ring path flushes the buffer and keeps buffering (§below); only its gap-wedge fallback writes a single line straight through. Both reuse the same eager `format_line` (§9).
...
int  ano_log_enqueue(log_types_t level, const char *file, int line, const char *fmt, ...)
        __attribute__((format(printf, 4, 5)));      // checks args vs fmt; pair with -Wformat-nonliteral
                                // 0 enqueued; 1 written immediately (ring was full).
...
- Full ring: flood past capacity with no intervening flush and assert every record still appears (each overflow line written immediately on the producer), since write-through never drops.
