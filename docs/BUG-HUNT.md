...
The trivial-fix wave 〜 2026-07-25. After the black-box suite consolidation, the five open entries whose repairs were mechanically small and decision-free were closed 〜 `apply.c:125`, `render_slots.c:92`, `swapchain.c:110`, `flat.c:244` and the `compute.c:80` family 〜 with the accounting in `docs/BUGS.md` beside the census. One note belongs here rather than there, because it is a resource-manager marker and not a settled fix: the unwind work surfaced `loadFile` (`pipeline.c:63`) as not clean-on-failure 〜 a refused load returned a dangling non-NULL `buffer->data`, and its free crossed allocators in name, saved only by the TU-wide mimalloc override 〜 and the same-day repair (prologue-total out-params, `ano_aligned_free` by name) plus the pipeline builders' defensive re-inert of refused buffers at their fail labels are a temporary workaround, not an endpoint. The root cause is that `loadFile` is a stray file-loading operation living inside the Vulkan backend, exactly the load-side antipattern the resource-manager plan already names ("shaders via `ano_res_load`, kill `loadFile` antipatterns" 〜 `docs/BUGS.md`, Three layers), and the resource manager retires it: shader bytes arrive through `ano_res_load`, `loadFile` is deleted, and the workaround surface 〜 the totality patch and every caller-side re-inert 〜 goes with it. Until then nothing new should be built on `loadFile`, and no further source action is required.
